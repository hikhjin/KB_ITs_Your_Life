<template>
    <div class="card card-body">
        ...
        <button @click="goReplace">go about(replace)</button>
        <button @click="goPrevPage">이전 페이지(back)</button>
...
<script setup>
//script setup에서 route객체 활용하는 방법
import {useRoute, useRouter} from 'vue-router';
//useRoute() : 해당 페이지의 url 정보를 담고 있는 객체(로컬 정보) -> 현재 페이지의 path, query, param 정보를 조회 할 때 사용
//useRouter(router) : router/index에서 생성한 router 정보로 모든 라우팅 정보를 담고 있는 객체 -> 라우터 전용 메소드 활용 ex)push, go, replace, back
...
const goPush = () => {
    //router.push('/about'); url을 통해 요청하는 방법
    router.push({path:'/about'}); // path를 통해 요청하는 방법
}
const goReplace = () => {
